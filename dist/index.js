...
(() => {
  ...
  var ApiSync = class {
    ...
    save(data) {
      const { id } = data;
      if (id) return axios_default.patch(`${this.url}/${id}`, data);
  ...
  var Collection = class {
    constructor(url, deserialization) {
      this.url = url;
      this.deserialization = deserialization;
      this.models = [];
      this.event = new Eventing();
  ...
  var User = class _User extends Model {
    ...
    static buildCollection() {
      return new Collection(BaseUrl, (json) => _User.build(json));
    }
    setRandomAge() {
